...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			$output .= '<div class="pre-menu"><ul role="menu" class="dropdown-menu">';
		}
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
				...
				if ( $args->has_children && 0 === $depth ) {
					if (wp_is_mobile()){
						$atts['href']           = '#';
						$atts['onclick'] = 'activatemenu' . $item->ID .'();';
						$atts['id'] = 'a-menu-item-' . $item->ID;
					}
					else{
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$output .= '<ul' . ( $menu_id ? ' id="' . esc_attr( $menu_id ) . '"' : '' ) . ( $menu_class ? ' class="' . esc_attr( $menu_class ) . '"' : '' ) . '>';
					$output .= '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_html__( 'Add a menu', 'scroller-it' ) . '</a></li>';
				$output .= '</ul></div>';
				if ( $container ) {
